...
function SelectedSlicePanel({ slices, total, selectedSliceId, onSelect }: { slices: Slice[]; total: number; selectedSliceId: string | null; onSelect: (id: string) => void }) {
  const items = slices.map((slice) => {
    const percent = percentFor(slice, total) * 100;
    return {
      id: slice.id,
      name: slice.name,
      amount: slice.amount,
      detail: `${percent.toFixed(percent > 0 && percent < 1 ? 1 : 0)}% · ${TOPPING_LABELS[slice.topping]}`,
    };
  });

  return (
    <div className="mobile-selected-panel custom-selected-panel">
      <BucketPicker items={items} selectedId={selectedSliceId} onSelect={onSelect} formatAmount={money} />
    </div>
  );
}

...
function loadState() {
  for (const key of [STORAGE_KEY, ...LEGACY_STORAGE_KEYS]) {
    ...
    try {
      ...
      const activePie = planningVisible ? storedPie(parsed.activePie) : "current";
      return {
        current,
        planning,
        planningVisible,
...
function cloudStatusText(status: CloudStatus, message: string): string {
...
function PizzaWindowChart({ slices, title }: { slices: Slice[]; title: string }) {
  ...
  return (
    <div className="pizza-chart-wrap" aria-label={`${title} allocation chart`}>
          ...
          {segments.length === 0 ? (
            ...
            segments.map((segment, index) => (
              <g key={segment.slice.id} clipPath={`url(#${chartId}-clip-${index})`}>
                <image href={PIZZA_TEXTURES[segment.slice.topping]} x="16" y="16" width="288" height="288" preserveAspectRatio="xMidYMid slice" />
              </g>
            ))
          )}
...
function SliceList({ slices, total, selectedSliceId, onSelect }: { slices: Slice[]; total: number; selectedSliceId: string | null; onSelect: (id: string) => void }) {
        ...
        return (
          <button key={slice.id} type="button" className={`slice-row ${selectedSliceId === slice.id ? "selected" : ""}`} onClick={() => onSelect(slice.id)}>
            <span>
              ...
              <small>{TOPPING_LABELS[slice.topping]}</small>
            </span>
...
function App() {
  ...
  function exportJson() {
    const payload = JSON.stringify(currentPayload(), null, 2);
            ...
            <div className="workbench-grid json-tools data-tools">
              ...
              <div className="cloud-data-panel">
                <div className="cloud-heading">
                  ...
                  <span>{session?.user ? session.user.email ?? "Signed in" : "Email sign-in"}</span>
                </div>

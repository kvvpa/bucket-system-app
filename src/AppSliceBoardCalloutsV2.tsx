...
function PizzaWindowChart({
  ...
  return (
    <div className="pizza-chart-wrap callout-chart-wrap" aria-label={`${title} allocation chart`}>
      <svg className="pizza-chart-svg callout-chart-svg callout-chart-svg-v2" viewBox="0 0 380 300" role="img" aria-label={`${title} proportional raster pizza chart`}>
        <defs>
          ...
          <mask id={`${chartId}-donut-mask`} maskUnits="userSpaceOnUse">
            <rect x="0" y="0" width="380" height="300" fill="black" />
        ...
        <g className="callout-layer">
          {labelSegments.map((segment) => {
            ...
            const labelY = Math.max(26, Math.min(274, lineBend.y));
...
function App() {
  ...
  function selectSibling(direction: -1 | 1) {
    const candidates = displaySlices.filter((slice) => slice.amount > 0 || slice.id === selectedSliceId);
    if (!candidates.length) return;
    const currentIndex = Math.max(0, candidates.findIndex((slice) => slice.id === selectedSliceId));
    const nextIndex = (currentIndex + direction + candidates.length) % candidates.length;
    setSelectedSliceId(candidates[nextIndex].id);
  }
  ...
  return (
    <main className="app-shell">
      <div className="dashboard-frame">
        ...
        <section className="screen-card chart-stage callout-stage callout-stage-v2">

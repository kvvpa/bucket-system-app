...
const TOPPING_OPTIONS: { value: ToppingKind; label: string; emoji: string }[] = [
...
function cleanChartId(value: string) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "slice-board-chart";
}
...
function starterState() {
  const starterPie = { id: newId("current-pie"), name: "Monthly Budget", slices: cloneSlices(STARTER_CURRENT) };
...
function loadState() {
  for (const key of [STORAGE_KEY, ...LEGACY_STORAGE_KEYS]) {
    ...
    try {
      ...
      const pies: Pie[] = [{ id: newId("current-pie"), name: "Monthly Budget", slices: current }];
...
function cloudStatusText(status: CloudStatus, message: string) {
  ...
  return isSupabaseConfigured ? "Cloud tools are ready for Supabase verification." : "Cloud setup is waiting for Supabase env values.";
}
...
function PizzaWindowChart({ slices, title, selectedSliceId, onSelect }: {
  ...
  const chartId = `slice-board-${cleanChartId(title)}`;
  const cx = 190;
  const cy = 146;
  ...
  return (
    <div className="vnext-pizza-wrap" aria-label={`${title} allocation chart`}>
      <svg className="vnext-pizza-svg" viewBox="0 0 380 292" role="img" aria-label={`${title} proportional raster pizza chart`}>
        <defs>
        ...
        <circle cx={cx} cy={cy} r={outerRadius + 4} className="vnext-pizza-glow" />
        <g mask={`url(#${chartId}-donut-mask)`}>
          {segments.length === 0 ? (
            ...
            segments.map((segment, index) => (
              <g key={segment.slice.id} className="vnext-pizza-segment" clipPath={`url(#${chartId}-clip-${index})`} onClick={() => onSelect(segment.slice.id)}>
          ...
          <circle cx={cx} cy={cy} r={outerRadius} fill={`url(#${chartId}-shade)`} className="vnext-pizza-shade" />
        </g>
        ...
        {segments.map((segment) => {
          ...
          return <line key={`${segment.slice.id}-separator`} x1={inner.x} y1={inner.y} x2={outer.x} y2={outer.y} className="vnext-pizza-separator" />;
        })}

        <circle cx={cx} cy={cy} r={innerRadius} className="vnext-pizza-hole" />
        <circle cx={cx} cy={cy} r={outerRadius} className="vnext-pizza-stroke" />

        <g className="vnext-callout-layer">
          {labelSegments.map((segment) => {
            ...
            return (
              <g key={`${segment.slice.id}-callout`} className={`vnext-callout ${selected ? "selected" : ""}`} onClick={() => onSelect(segment.slice.id)}>
                <line x1={lineStart.x} y1={lineStart.y} x2={lineBend.x} y2={lineBend.y} className="vnext-callout-line" />
                <line x1={lineBend.x} y1={lineBend.y} x2={labelX} y2={labelY} className="vnext-callout-line" />
                <text x={labelX} y={labelY - 3} textAnchor={rightSide ? "end" : "start"} className="vnext-callout-name">{shortLabel(segment.slice.name)}</text>
                <text x={labelX} y={labelY + 12} textAnchor={rightSide ? "end" : "start"} className="vnext-callout-meta">{percent.toFixed(percent > 0 && percent < 1 ? 1 : 0)}%</text>
              </g>
            );
          })}
        </g>
      </svg>
      <div className="vnext-center-label">
...
function SelectedSlicePanel({ slices, total, selectedSliceId, onPrevious, onNext }: {
  ...
  if (!selected) return <div className="vnext-selected-strip">No slices yet.</div>;
  const percent = percentFor(selected, total) * 100;

  return (
    <div className="vnext-selected-strip">
      <button type="button" onClick={onPrevious} aria-label="Previous bucket">Prev</button>
      <div className="vnext-selected-main">
        <span>
        ...
        <span>
          ...
          <small>{percent.toFixed(percent > 0 && percent < 1 ? 1 : 0)}% of pie / {TOPPING_LABELS[selected.topping]}</small>
        </span>
      </div>
      <button type="button" onClick={onNext} aria-label="Next bucket">Next</button>
    </div>
  );
}

export default function AppSliceBoardVNext() {
  ...
  const [settingsPanel, setSettingsPanel] = useState<SettingsPanel>("home");
  ...
  function exportJson() {
    ...
    setImportMessage("Exported local backup JSON.");
  }
  ...
  function applyImportedPayload(parsed: StoredState) {
    const loaded = (() => {
      ...
      return [{ id: newId("current-pie"), name: "Monthly Budget", slices: normalizeSlices(parsed.current ?? parsed.currentPie ?? parsed.slices, STARTER_CURRENT) }];
    })();
    setPies(loaded);
  ...
  async function saveToCloud() {
    if (!supabase || !session?.user) {
      setSettingsPanel("cloud-sign-in");
      return;
    }
    setCloudStatus("working");
    setCloudMessage("");
  ...
  return (
    <main className="vnext-shell">
      <div className="vnext-frame">
        <header className="vnext-hero">
          <img className="vnext-logo" src={logoPng} alt="Slice Board logo" />
          <div>
            <p className="vnext-eyebrow">Slice Board</p>
            <h1>Slice Board</h1>
            <p>Your money, by the slice.</p>
          </div>
          <button type="button" className="vnext-gear" onClick={() => setSettingsOpen(true)} aria-label="Open settings">⚙</button>
        </header>

        <section className="vnext-stats" aria-label="Active pie summary">
        ...
        >
          <div className="vnext-chart-heading">
            <div>
              <p className="vnext-eyebrow">Current pie</p>
              <h2>{activePie?.name ?? "Monthly Budget"}</h2>
            </div>
            <span>{activePieNumber} / {MAX_PIES}</span>
          </div>
          ...
          <p className="vnext-swipe-hint">Swipe to switch pies.</p>
        </section>
        ...
        <section className="vnext-workbench">
          {view === "pie" && (
            <div className="vnext-pie-tools">
              <div className="vnext-segmented" aria-label="Amount change mode">
              ...
              <div className="vnext-amount-row">
                <label>
                  <span>Amount</span>
                  <input inputMode="decimal" placeholder="$0.00" value={toolAmount} onChange={(event: ChangeEvent<HTMLInputElement>) => setToolAmount(event.target.value)} />
                </label>
          ...
          {view === "edit" && (
            <div className="vnext-edit-tools">
              <label className="vnext-big-field">
                <span>Pie name</span>
                <input value={activePie?.name ?? ""} onChange={(event: ChangeEvent<HTMLInputElement>) => updatePieName(event.target.value)} />
              </label>
              <div className="vnext-two-actions">
                <button type="button" onClick={addPie} disabled={pies.length >= MAX_PIES}>Add Pie</button>
                <button type="button" className="danger" onClick={deleteActivePie} disabled={pies.length <= 1}>Delete Pie</button>
              </div>
              {selectedSlice && (
                <div className="vnext-edit-card">
                  <p className="vnext-eyebrow">Selected bucket editor</p>
                  <label>
                    <span>Name</span>
                    <input value={selectedSlice.name} onChange={(event: ChangeEvent<HTMLInputElement>) => updateSlice(selectedSlice.id, { name: event.target.value })} />
                  </label>
                  <div className="vnext-edit-pair">
                    <label>
                      <span>Amount</span>
                      <input inputMode="decimal" value={String(selectedSlice.amount)} onChange={(event: ChangeEvent<HTMLInputElement>) => updateSlice(selectedSlice.id, { amount: toAmount(event.target.value) })} />
                    </label>
                    <label>
                      <span>Topping</span>
                      <select value={selectedSlice.topping} onChange={(event: ChangeEvent<HTMLSelectElement>) => updateSlice(selectedSlice.id, { topping: event.target.value as ToppingKind })} disabled={selectedSlice.kind !== "named"}>
                        {(selectedSlice.kind === "named" ? TOPPING_OPTIONS : [{ value: selectedSlice.topping, label: TOPPING_LABELS[selectedSlice.topping], emoji: "" }]).map((option) => (
              ...
              <button type="button" className="vnext-add-bucket-row" onClick={() => setAddBucketOpen(true)}>+ Add Bucket</button>
            </div>
          )}
        </section>

        <nav className="vnext-bottom-nav" aria-label="Slice Board tools">
      ...
      {addBucketOpen && (
        <div className="vnext-sheet-backdrop" onClick={() => setAddBucketOpen(false)}>
          <section className="vnext-add-sheet" onClick={(event) => event.stopPropagation()} aria-label="Add bucket">
            <div className="vnext-sheet-handle" />
            <div className="vnext-sheet-title">
              <h2>Add Bucket</h2>
              <button type="button" onClick={() => setAddBucketOpen(false)} aria-label="Close add bucket">×</button>
            </div>
            <label className="vnext-big-field">
              <span>Bucket name</span>
              <input value={newName} onChange={(event: ChangeEvent<HTMLInputElement>) => setNewName(event.target.value)} placeholder="e.g. Groceries" />
            </label>
            <label className="vnext-big-field">
              <span>Amount</span>
              <input inputMode="decimal" value={newAmount} onChange={(event: ChangeEvent<HTMLInputElement>) => setNewAmount(event.target.value)} placeholder="$0.00" />
            </label>
            <div className="vnext-topping-picker" aria-label="Visual topping selector">
              <span>Topping</span>
              <div>
                {TOPPING_OPTIONS.map((option) => (
                  <button key={option.value} type="button" className={newTopping === option.value ? "active" : ""} onClick={() => setNewTopping(option.value)} aria-label={option.label}>
            ...
            <button type="button" className="vnext-primary-large" onClick={addBucket}>Save Bucket</button>
          </section>
        </div>
      )}

      <section className={`vnext-settings-page ${settingsOpen ? "open" : ""}`} aria-hidden={!settingsOpen}>
        <div className="vnext-settings-header">
          <button type="button" onClick={closeSettings} aria-label="Close settings">‹</button>
          <div>
            <p className="vnext-eyebrow">Slice Board</p>
            <h2>Settings</h2>
          </div>
        ...
        {settingsPanel === "home" && (
          <div className="vnext-settings-list">
            <section>
              <h3>Backup & Sync</h3>
              <button type="button" onClick={saveToCloud}><span>☁</span><strong>Cloud Save<small>Save your board to Supabase.</small></strong><b>›</b></button>
              <button type="button" onClick={loadFromCloud}><span>☁</span><strong>Cloud Load<small>Restore your saved cloud board.</small></strong><b>›</b></button>
              {session ? (
                <button type="button" onClick={signOutCloud}><span>◎</span><strong>Sign Out<small>{session.user.email ?? "Signed in"}</small></strong><b>›</b></button>
              ) : (
                <button type="button" onClick={() => setSettingsPanel("cloud-sign-in")}><span>◎</span><strong>Sign In / Cloud Backup<small>Email magic link sign-in.</small></strong><b>›</b></button>
              )}
            </section>

            <section>
              <h3>Import / Export</h3>
              <button type="button" onClick={exportJson}><span>⇧</span><strong>Export Backup<small>Download a local JSON backup.</small></strong><b>›</b></button>
              <button type="button" onClick={() => setSettingsPanel("import")}><span>⇩</span><strong>Import Backup<small>Paste JSON and restore.</small></strong><b>›</b></button>
            </section>

            <section>
              <h3>Advanced</h3>
              <button type="button" onClick={() => setSettingsPanel("json")}><span>{"{}"}</span><strong>JSON Data<small>View or copy current board data.</small></strong><b>›</b></button>
            </section>

            <p className="vnext-status-copy">{cloudStatusText(cloudStatus, cloudMessage || importMessage)}</p>
          </div>
        )}

        {(settingsPanel === "import" || settingsPanel === "json") && (
          <div className="vnext-settings-detail">
            <button type="button" className="vnext-back-row" onClick={() => setSettingsPanel("home")}>‹ Settings</button>
            <h3>{settingsPanel === "import" ? "Import Backup" : "JSON Data"}</h3>
            <textarea value={settingsPanel === "json" && !importText ? JSON.stringify(payload, null, 2) : importText} onChange={(event: ChangeEvent<HTMLTextAreaElement>) => setImportText(event.target.value)} />
            {settingsPanel === "import" ? (
              <button type="button" className="vnext-primary-large" onClick={importJson}>Import JSON</button>
            ) : (
              <button type="button" className="vnext-primary-large" onClick={() => navigator.clipboard?.writeText(JSON.stringify(payload, null, 2))}>Copy JSON</button>
            )}
            <p className="vnext-status-copy">{importMessage || "Keep JSON tools tucked away unless you need them."}</p>
          </div>
        )}

        {settingsPanel === "cloud-sign-in" && (
          <div className="vnext-settings-detail">
            <button type="button" className="vnext-back-row" onClick={() => setSettingsPanel("home")}>‹ Settings</button>
            <h3>Cloud Backup</h3>
            <label className="vnext-big-field">
              <span>Email</span>
              <input type="email" value={cloudEmail} onChange={(event: ChangeEvent<HTMLInputElement>) => setCloudEmail(event.target.value)} placeholder="you@example.com" />
            </label>
            <button type="button" className="vnext-primary-large" onClick={sendSignInLink} disabled={!supabase || cloudStatus === "working"}>Send Magic Link</button>
            <p className="vnext-status-copy">{cloudStatusText(cloudStatus, cloudMessage)}</p>
          </div>
        )}
      </section>
    </main>
